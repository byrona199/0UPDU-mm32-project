...
void CAN_AutoCfg_BaudParam(CAN_Peli_InitTypeDef* init_struct, u32 src_clk, u32 baud)
{
    ...
    u32 remain = 0, sumPrescaler = 0;
	  //u32 tesg1, tesg2;
	  //u32 total_time_segment = 0;
    //const float target_sample_point = 0.875; // 目標取樣點為 87.5%
...
/*
		// 計算 TESG1 和 TESG2，根據目標取樣點 87.5%
    total_time_segment = record - 3;
    //tesg2 = total_time_segment * (1 - target_sample_point);
		tesg2 = (u32)(total_time_segment * (1 - target_sample_point) + 0.5);  // 四捨五入
    tesg1 = total_time_segment - tesg2;

    // 檢查並調整 TESG1 和 TESG2 使其符合範圍
    if (tesg1 > 15) {
        tesg1 = 15;
        tesg2 = total_time_segment - tesg1;  // Adjust TESG2 accordingly
    }

    if (tesg2 > 7)
			  tesg2 = 7;  // TESG2 不能超過 8

    // 設置 TESG1 和 TESG2
    init_struct->TESG1 = tesg1;
    init_struct->TESG2 = tesg2;
*/
}
